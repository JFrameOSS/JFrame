...
/**
 * Tests for {@link JpaSearchSpecification} in jframe-spring-jpa.
 *
 * <p>Verifies that the Spring JPA {@link JpaSearchSpecification} correctly extends
 * {@link BaseSearchSpecification}, implements Spring Data's {@link Specification},
 * and delegates predicate construction to the base class.
 */
@DisplayName("Spring JPA - JpaSearchSpecification")
public class JpaSearchSpecificationTest extends UnitTest {
    ...
    private static JpaSearchSpecification<Object> aSpecWithEmptyCriteria() {
        return new JpaSearchSpecification<>(Collections.emptyList());
    }

    private static JpaSearchSpecification<Object> aSpecWithNullCriteria() {
        return new JpaSearchSpecification<>(null);
    }
    ...
    public void shouldCreateInstanceWithEmptyCriteriaList() {
        ...
        // When: Creating a JpaSearchSpecification with empty criteria
        final JpaSearchSpecification<Object> spec = new JpaSearchSpecification<>(Collections.emptyList());
    ...
    public void shouldCreateInstanceWithNullCriteriaList() {
        ...
        // When: Creating a JpaSearchSpecification with null criteria
        final JpaSearchSpecification<Object> spec = new JpaSearchSpecification<>(null);
    ...
    public void shouldCreateInstanceWithNonEmptyCriteriaList() {
        ...
        // When: Creating a JpaSearchSpecification
        final JpaSearchSpecification<Object> spec = new JpaSearchSpecification<>(criteria);
    ...
    public void shouldImplementSpringDataSpecificationInterface() {
        // Given: A JpaSearchSpecification instance
        final JpaSearchSpecification<Object> spec = aSpecWithEmptyCriteria();

        // When: Checking the type hierarchy

        // Then: Must be an instance of Spring Data's Specification
    ...
    public void shouldImplementCoreSearchSpecificationInterfaceViaInheritance() {
        ...
        // Then: Must be assignable to the core SearchSpecification interface
        assertThat(spec, is(instanceOf(SearchSpecification.class)));
    }

    @Test
    @DisplayName("Should be assignable where Specification is expected")
    public void shouldBeAssignableWhereSpecificationIsExpected() {
        // Given: A JpaSearchSpecification
        final JpaSearchSpecification<Object> spec = aSpecWithEmptyCriteria();

        // When: Assigning to a Specification reference
        final Specification<Object> springSpec = spec;

        // Then: Assignment succeeds and reference is not null
        assertThat(springSpec, is(notNullValue()));
    }
    ...
    public void shouldDelegateToPredicateToBaseClassLogic() {
        ...
        // When: Calling toPredicate via the Spring Specification reference
        final Specification<Object> springSpec = spec;
        final Predicate result = springSpec.toPredicate(root, criteriaQuery, criteriaBuilder);
    ...
    public void shouldReturnPredicateWhenToPredicateIsCalledWithNullCriteria() {
        ...
        final JpaSearchSpecification<Object> spec = aSpecWithNullCriteria();
        when(criteriaBuilder.conjunction()).thenReturn(predicate);
