...
class EcsFieldsTest extends UnitTest {
    ...
    void shouldSetMdcFieldWhenTaggingWithLong() {
        ...
        assertThat(MDC.get(REQUEST_ID.getKey()), is("123"));
    }
    ...
    void shouldSetMdcFieldWhenTaggingWithLongMaxValue() {
        ...
        assertThat(MDC.get(HTTP_STATUS.getKey()), is(Long.toString(Long.MAX_VALUE)));
    }
    ...
    void shouldSetMdcFieldWhenTaggingCloseableWithLong() {
        ...
        try (AutoCloseableEcsField field = EcsFields.tagCloseable(HTTP_STATUS, 123L)) {
            // Then: Field is set within the try block
            assertThat(EcsFields.get(HTTP_STATUS), is("123"));
        }
    ...
    void shouldChainFieldsWhenUsingAndWithLong() {
        ...
        try (AutoCloseableEcsField field = EcsFields.tagCloseable(HTTP_STATUS, 200).and(TX_ID, 456L)) {
            // Then: Both fields are set within the try block
            assertThat(EcsFields.get(HTTP_STATUS), is("200"));
            assertThat(EcsFields.get(TX_ID), is("456"));
        }
